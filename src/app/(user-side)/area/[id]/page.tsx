...
import { StarIcon, MapPinIcon, BuildingOfficeIcon, HomeIcon, UserGroupIcon } from '@heroicons/react/24/solid'
...
const DetailsCity=async({ params }: { params: { id: string } })=>{
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      {/* Location Header */}
      <div className={`bg-gradient-to-br from-primary to-primary-light  rounded-lg shadow-lg overflow-hidden mb-8`}>
        <div className="px-6 py-12 sm:px-10 sm:py-16 text-white">
          <h1 className="text-3xl font-bold sm:text-4xl mb-2">{alldata.cityName}</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-white/90">
            <div>
              <p className="text-sm uppercase tracking-wider opacity-75">Average Price</p>
              <p className="text-xl font-semibold mt-1">{alldata.details?.averagePrice}</p>
            </div>
            <div>
              <p className="text-sm uppercase tracking-wider opacity-75">Popularity</p>
              <p className="text-xl font-semibold mt-1">{alldata.details?.popularity}</p>
            </div>
            <div>
              <p className="text-sm uppercase tracking-wider opacity-75">Available Properties</p>
              <p className="text-xl font-semibold mt-1">{alldata.details?.availableProperties}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900">About Milan</h2>
            </div>
        ...
        <div>
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900">Area Details</h2>
            </div>
            <div className="p-6">
            ...
            <dl className="grid grid-cols-1 gap-y-4">
                ...
                <div key={index}>
                  <dt className="text-sm font-medium text-gray-500">{data.heading}</dt>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden mb-12">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Types of Property in Milan </h2>
        </div>
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {alldata.details?.types.map((data,index)=><div key={index} className="bg-gray-50 p-4 rounded-lg">
              <div className="bg-blue-100 rounded-full w-12 h-12 flex items-center justify-center mb-3">
                <HomeIcon className="h-6 w-6 text-blue-600" />
              </div>
              <h3 className="text-lg font-medium text-gray-900 mb-1">{data.heading}</h3>
              <p className="text-gray-600 mb-2">{data.content}</p>
              <p className="text-sm text-gray-500">Average Price: <span className="font-medium">{data.price}</span></p>
            </div>)}
      ...
      <div className="bg-primary rounded-lg shadow-lg overflow-hidden">
        <div className="px-6 py-8 sm:px-10 sm:py-12 md:flex md:items-center md:justify-between">
          <div className="text-center md:text-left max-w-lg mx-auto md:mx-0">
            <h2 className="text-2xl font-bold text-white sm:text-3xl">Ready to find your ideal home in Milan ?</h2>
            <p className="mt-3 text-lg text-primary-light">
              Our top rated agents specialize in this area and can help you find the perfect property.
            </p>
          </div>
