...
import { editProperetyApi, findSinglePropertyAPi } from '@/src/lib/api_service_client/agent_agencies_service/property_handler';
...
const EditFeaturedProperty=({params}:{ params: Promise<{ id: string }> })=>{
    ...
    const initialValues={name:allData?.name||'',price:allData?.price||"",room:allData?.room||"",city:allData?.city.id||"",bathroom:allData?.bathroom||"",sm:allData?.sm||"",imageUrl:allData?.imageUrl||''}
...
<div >
  {showLoader&&<Loader  />}
          ...
          <h2 className="text-[24px] font-semibold text-center text-gray-800 mb-6">Edit Property Details</h2>
          ...
          <div className="mb-4">
            ...
            <option value={allData.city.id} >{allData.city.cityName}</option>
    ...
    <option key={idx} value={city.id}>
      {city.cityName}
          ...
          <div className="mb-4">
            ...
            />
            <div className="h-1">
                {errors.room && touched.room ? <div className="text-red-500 text-xs mt-1 ">{errors.room}</div> : null}
                </div>
          </div>
          <div className="mb-4">
            ...
            />
            <div className="h-1">
                {errors.sm && touched.sm ? <div className="text-red-500 text-xs mt-1 ">{errors.sm}</div> : null}
                </div>
          </div>
          ...
          <div>
            ...
            />
            <div className="h-1">
                {errors.bathroom && touched.bathroom ? <div className="text-red-500 text-xs mt-1 ">{errors.bathroom}</div> : null}
                </div>
          </div>
          <br/>
          <div>
