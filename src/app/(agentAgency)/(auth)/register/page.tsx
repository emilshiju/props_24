...
import {
  requestOtpApi,
  verifyOtpApi,
} from "@/src/lib/api_service_client/user_service/otp_handler";
import { toast } from "react-hot-toast";
import {
  checkEmailApi,
  registerUserApi,
} from "@/src/lib/api_service_client/user_service/auth_handler";
...
const Register = () => {
  ...
  const handleSubmit = async (
    ...
    if (ress.status) {
      setAllFormData({
        userName: values_data.userName,
        email: values_data.email,
        password: values_data.password,
        confirmPassword: values_data.confirmPassword,
        role: values_data.role,
      });
  ...
  const onSetDigit = (
    ...
    if (
      value &&
      inputRefNext.current &&
      index < inputRefNext.current.length - 1
    ) {
